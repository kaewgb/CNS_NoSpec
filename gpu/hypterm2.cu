...
__global__ void gpu_hypterm_x_stencil_kernel2(
	global_const_t *g,	// i:
	double *cons,		// i:
	double *q,			// i:
	double *flux		// o:
...
#define	s_q(i)			s_q[threadIdx.y+g->ng][threadIdx.x+g->ng+(i)]
...
#define	s_q(i)			s_q[threadIdx.y+g->ng+(i)][threadIdx.x+g->ng]
...
#undef	s_cons
		...
		flux[idx + irho*g->comp_offset_padded] = 4.0;
		flux[idx + imx *g->comp_offset_padded] = 4.0;
		flux[idx + imy *g->comp_offset_padded] = 4.0;
		flux[idx + imz *g->comp_offset_padded] = 4.0;
		flux[idx + iene*g->comp_offset_padded] = 4.0;
...
//	gpu_hypterm_xy_stencil_kernel<<<grid_dim_xy_stencil, block_dim_xy_stencil>>>(d_const, d_cons, d_q, d_flux);
	...
	dim3 block_dim_x_stencil(BLOCK_SMALL, BLOCK_SMALL, 1);
	dim3 grid_dim_x_stencil(CEIL(h_const.dim[0], BLOCK_SMALL), CEIL(h_const.dim[1], BLOCK_SMALL), h_const.dim[2]);
	gpu_hypterm_x_stencil_kernel2<<<grid_dim_x_stencil, block_dim_x_stencil>>>(d_const, d_cons, d_q, d_flux);
